...
// Version: v0.4.0
/**
 * ParametersEditorComponent (modal)
 * - Globals & Levels editor for construction tree parameters.
 * - Primitive fields use text/number inputs.
 * - SPECIAL: `ga` in Globals is expanded into separate fields (no JSON).
 * - SPECIAL: `tm` in Globals is shown in an expandable box with separate fields.
 * - Other object/array fields use a JSON textarea with live validation.
 * - Levels sidebar (scrollable), per-level field editor (scrollable).
 * - Add/Insert/Delete levels with index reflow.
 * - Import/Export (Globals / Levels) independently.
 * - Locked schema: keys from backend defaults; only values are editable.
 */
...
export class ParametersEditorComponent {
  ...
  /** Expand/collapse for the tm box. */
  tmOpen = signal(true);
  ...
  onImportGlobals(ev: Event) {
    ...
    reader.onload = () => {
      ...
      this.globalJsonErrs = {};
      // Re-open tm if it exists so user sees it
  ...
  /** Update a single nested field inside a globals group (e.g., ga.population_size). */
